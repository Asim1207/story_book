...
// @desc    Export a story project as a PDF
// @route   POST /api/projects/:id/export/pdf
// @access  Private
const exportStoryAsPdf = async (req, res) => {
    try {
        const project = await StoryProject.findById(req.params.id);
        if (!project) {
            return res.status(404).json({ message: 'Project not found.' });
        ...
        const pdfFilename = await pdfService.generatePdfFromStory(project);

        const [signedUrl] = await storage.bucket(process.env.GCS_BUCKET_NAME).file(pdfFilename).getSignedUrl({
            action: 'read',
...
module.exports = { createStoryProject, getStoryProjects, updateStoryProject, deleteStoryProject, updatePageText, regeneratePageImage, uploadPageImage, addPage, deletePage, upscalePageImage, exportStoryAsPdf };
